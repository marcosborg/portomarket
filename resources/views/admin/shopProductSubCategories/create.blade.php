...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shop-product-sub-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop_product_category') ? 'is-invalid' : '' }}" name="shop_product_category_id" id="shop_product_category_id" required>
                    @foreach($shop_product_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('shop_product_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
