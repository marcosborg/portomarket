...
@section('content')
@can('shop_product_sub_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.shop-product-sub-categories.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.shopProductSubCategory.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.shopProductSubCategory.title_singular') }} {{ trans('global.list') }}
    </div>
                ...
                <tbody>
                    @foreach($shopProductSubCategories as $key => $shopProductSubCategory)
                        <tr data-entry-id="{{ $shopProductSubCategory->id }}">
                            ...
                            <td>
                                @can('shop_product_sub_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.shop-product-sub-categories.show', $shopProductSubCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('shop_product_sub_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.shop-product-sub-categories.edit', $shopProductSubCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('shop_product_sub_category_delete')
                                    <form action="{{ route('admin.shop-product-sub-categories.destroy', $shopProductSubCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
